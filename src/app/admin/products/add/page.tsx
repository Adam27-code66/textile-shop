...
import { PlusCircle, Upload, ArrowLeft, CheckCircle2, AlertCircle, Image as ImageIcon } from 'lucide-react';
...
export default function AdminAddProductPage() {
  ...
  const [careInstructions, setCareInstructions] = useState('Machine wash cold / Dry clean recommended.');
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (json.success && json.url) {
        ...
        // Fallback file reader if server upload offline
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !price || !category) {
      setErrorMessage('Please fill in Product Name, Price, and Category.');
      return;
    }

    setIsSubmitting(true);

    const finalImage = imageUrl.trim() || 'https://images.unsplash.com/photo-1521572163474-6864f9cf17ab?w=800&q=80';
    ...
    if (result.success) {
      setSuccessMessage('Product created successfully! It is now live on the customer website.');
      setTimeout(() => {
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <h1 className="text-2xl font-extrabold text-white tracking-tight">+ Add New Clothing Item</h1>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="md:col-span-2">
            ...
            <input
              ...
              placeholder="e.g. Royal Oxford Embroidered Shirt"
              className="w-full bg-white/[0.04] border border-white/[0.08] py-3 px-4 text-sm text-white placeholder:text-[#A6A6B0]/50 focus:outline-none focus:border-[#8B3DFF] transition-colors rounded-lg"
            />
          </div>
          ...
          <div className="md:col-span-2">
            ...
            <textarea
              ...
              placeholder="Crafted from 100% long-staple cotton with a relaxed fit..."
              className="w-full bg-white/[0.04] border border-white/[0.08] p-4 text-sm text-white placeholder:text-[#A6A6B0]/50 focus:outline-none focus:border-[#8B3DFF] transition-colors rounded-lg"
            />
          </div>
          ...
          <div className="md:col-span-2 space-y-3">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <p className="text-[11px] text-[#A6A6B0] mb-1.5">Option A: Upload Image File</p>
                <label className="flex flex-col items-center justify-center border-2 border-dashed border-white/[0.1] hover:border-[#8B3DFF]/60 bg-white/[0.02] p-6 rounded-lg cursor-pointer transition-colors">
                  ...
                  <span className="text-xs text-white font-medium">
                    {isUploading ? 'Uploading Image...' : 'Click to Upload Clothing Image'}
                  </span>
                  <span className="text-[10px] text-[#A6A6B0] mt-1">PNG, JPG, WEBP up to 10MB</span>
                  <input
                    type="file"
                    accept="image/*"
              ...
              <div>
                <p className="text-[11px] text-[#A6A6B0] mb-1.5">Option B: Image URL</p>
                <input
                  type="text"
                  value={imageUrl}
                  onChange={(e) => setImageUrl(e.target.value)}
                  placeholder="https://images.unsplash.com/photo-..."
                  className="w-full bg-white/[0.04] border border-white/[0.08] py-3 px-4 text-xs text-white placeholder:text-[#A6A6B0]/50 focus:outline-none focus:border-[#8B3DFF] transition-colors rounded-lg"
                />
                ...
                {imageUrl && (
                  <div className="mt-3 flex items-center gap-3 p-2 bg-white/[0.02] border border-white/[0.06] rounded-lg">
                    ...
                    <span className="text-[11px] text-emerald-400 font-medium flex items-center gap-1">
                      <ImageIcon size={14} /> Image Preview Loaded
                    </span>
                  </div>
                )}
              </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. 100% Egyptian Cotton, Satin Finish"
              className="w-full bg-white/[0.04] border border-white/[0.08] py-3 px-4 text-xs text-white placeholder:text-[#A6A6B0]/50 focus:outline-none focus:border-[#8B3DFF] transition-colors rounded-lg"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Machine wash cold / Dry clean recommended."
              className="w-full bg-white/[0.04] border border-white/[0.08] py-3 px-4 text-xs text-white placeholder:text-[#A6A6B0]/50 focus:outline-none focus:border-[#8B3DFF] transition-colors rounded-lg"
            />
          </div>
